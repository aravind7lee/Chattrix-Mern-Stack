...
const LoginPage = () => {
  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      ...
      toast.error(
        err?.message || "Action failed. Check credentials or network."
      );
    } finally {
  ...
  return (
    <>
      {/* 
        Global, page-level fixes:
        - Zero out default margins
        - Prevent horizontal scroll (fixes right white strip)
        - Force a dark background on the *whole* page
        - Use dynamic viewport units so the background truly fills mobile screens
      */}
      <style>{`
        :root, html, body, #root { height: 100%; }
        html, body { margin: 0; padding: 0; overflow-x: hidden; background: #07070b; color-scheme: dark; }
        * { box-sizing: border-box; }
        body { -webkit-tap-highlight-color: transparent; }
        /* A utility we can rely on from the component: min-h that matches visual viewport */
        @supports (height: 100dvh) {
          .min-h-screen-fixed { min-height: 100dvh; }
        }
        @supports not (height: 100dvh) {
          /* Fall back to small/large viewport units where supported, then 100vh */
          @supports (height: 100svh) {
            .min-h-screen-fixed { min-height: 100svh; }
          }
          @supports not (height: 100svh) {
            .min-h-screen-fixed { min-height: 100vh; }
          }
        }
        /* Help on iOS/Android gesture areas so background color extends fully */
        .pad-safe { padding-bottom: env(safe-area-inset-bottom); }
      `}</style>

      {/* 
        Page wrapper:
        - "min-h-screen-fixed" ensures the background always fills the mobile viewport height (no bottom white).
        - "w-full overflow-x-hidden" kills any accidental horizontal scrolling (no right white strip).
        - A real background gradient applied directly here so it covers 100% of the page.
      */}
      <div
        className="relative min-h-screen-fixed w-full overflow-x-hidden isolate pad-safe flex items-center justify-center"
        style={{
          background:
            "radial-gradient(140% 100% at 50% 0%, #221a35 0%, #0f0c18 55%, #07070b 100%)",
        }}
      >
        <div className="w-full max-w-6xl grid grid-cols-1 lg:grid-cols-12 gap-8 items-center px-4 py-8">
          ...
          <main className="col-span-12 lg:col-span-6 flex items-center justify-center px-2">
            <form
              ...
              className="relative w-full max-w-md p-4 sm:p-6 md:p-8 rounded-2xl border border-white/10 backdrop-blur-xl shadow-2xl"
              style={{
                background:
            ...
            >
              ...
              <div
                ...
                style={{ height: "auto", willChange: "height" }}
              >
                {/* Step 1 */}
                <div
                  ref={step1Ref}
        ...
        <style>{`
          ...
          /* slightly tighter padding on narrow devices */
